...
def install_ffmpeg():
    ...
    try:
        ...
        if result.returncode == 0:
            print("ffmpeg is already available:", result.stdout.splitlines()[0])
    except (FileNotFoundError, subprocess.TimeoutExpired):
        print("ffmpeg not found, attempting installation...")
        try:
            subprocess.run(["apt-get", "install", "-y", "ffmpeg"], capture_output=True, text=True, timeout=120, check=True)
            print("ffmpeg installed successfully")
        except Exception as e:
            print(f"Installation failed: {e}")
    try:
        subprocess.run(["apt-get", "install", "-y", "fonts-noto-cjk"], capture_output=True, text=True, timeout=120, check=True)
        print("Noto CJK font installed")
    except Exception as e:
...
def render():
    ...
    if not KOREAN_FONT:
        return jsonify({"error": "No Korean font found on server"}), 500
    text = random.choice([
        '구독 \& 좋아요 눌러주세요',
        '구독하고 알림 받으세요',
        '좋아요와 구독 부탁드려요',
